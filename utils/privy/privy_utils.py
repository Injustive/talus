...
def _b64safe(value: bytes) -> str:
    return base64.urlsafe_b64encode(value).rstrip(b'=').decode('utf-8')


def random_string() -> str:
    random_bytes = os.urandom(36)
    return _b64safe(random_bytes)
...
def sha256safe(value: str) -> str:
    return _b64safe(sha256(value.encode('utf-8')))
...
def decrypt_share(b64_enc_data: str, b64_enc_iv: str, b64_key: str) -> bytes:
    key = b64d(b64_key)
    enc_iv = b64d(b64_enc_iv)
    enc_data = b64d(b64_enc_data)
    data = enc_data[:-16]
    tag = enc_data[-16:]
...
def encrypt_share(data: bytes, key: bytes) -> tuple[str, str]:
    iv = os.urandom(12)
    cipher = Cipher(algorithms.AES(key), modes.GCM(iv), backend=default_backend())
    e = cipher.encryptor()
    encrypted_data = e.update(data) + e.finalize()
    tag = e.tag
    enc_data = b64e(encrypted_data + tag)
    enc_iv = b64e(iv)
